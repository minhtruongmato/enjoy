...
class Localtion extends Admin_Controller {
    ...
    public function index() {
        ...
        $per_page = 10;
        $total_rows  = $this->localtion_model->count_searchs($this->data['keyword'],$this->data['area_id'],$this->data['is_hot'],'en');
        $config = $this->pagination_config(base_url('admin/'.$this->data['controller'].'/index'), $total_rows, $per_page, 4);
        $this->data['page'] = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
        $this->pagination->initialize($config);
        $this->data['page_links'] = $this->pagination->create_links();
        $this->data['result'] = $this->localtion_model->get_all_with_pagination_searchs('desc','en' , $per_page, $this->data['page'], $this->data['keyword'],$this->data['area_id'],$this->data['is_hot'],1);
